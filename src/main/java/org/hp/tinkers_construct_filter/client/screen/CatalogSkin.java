...
public final class CatalogSkin {
    ...
    public static void draw(LegacyGuiGraphics graphics, int sprite, int x, int y, int width, int height) {
        if (width <= 0 || height <= 0) {
            return;
        }
    ...
    public static void frame(LegacyGuiGraphics graphics, int sprite, int x, int y, int width, int height) {
        paint(graphics, sprite, x, y, width, height, false);
    }

    /** 每个面板最多九个四边形，中心拉伸而边角保持三个界面像素。 */
    private static void paint(LegacyGuiGraphics graphics, int sprite, int x, int y, int width, int height, boolean center) {
        if (width <= 0 || height <= 0) {
            return;
        }
    ...
    private static void blit(LegacyGuiGraphics graphics, int x, int y, int width, int height, int u, int v, int sourceWidth, int sourceHeight) {
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.setShaderTexture(0, TEXTURE);
        net.minecraft.client.gui.GuiComponent.blit(graphics.pose(), x, y, width, height, (float) u, (float) v, sourceWidth, sourceHeight, ATLAS_SIZE, ATLAS_SIZE);
    }
}
